...
async function loadImgList() {
    ...
    if ($('#img')) {
        $('#img>div').innerHTML = '';
        img.forEach(e => {
            $('#img>div').innerHTML += `<p onclick=navigator.clipboard.writeText(this.innerText) style='color:${dict[url[2]].includes(e.name)?"#aaa":"#fff"};'>${e.name}<button onclick=deleteImg('${e.name}')><i class="fa fa-trash"></i></button></p>`
        })
    }
}
