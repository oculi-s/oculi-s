...
async function getWidget() {
    ...
    if ($('aside>span')) { $('aside>span').innerHTML = auth.currentUser.email };
...
async function getData(x) {
    ...
    if (html.data()) {
        if (url[2] in html.data()) {
            ...
            if (ss.prp == 'false') { r = r.replace('%0A', '') }
            return de(r)
        } else { return create }
    } else { return create }
}

function setData(html) { var script = []; if (html.includes('<script')) { html = html.split('</script>'); for (var i = 0; i < html.length; i++) { if (html[i].includes('<script')) { script.push(html[i]) } else if ($('article')) { $('article').innerHTML = html[i] } } } else if ($('article')) { $('article').innerHTML = html } return script }
...
getWidget().then(async() => {
    ...
    if (ss.prp == 'true') { $('head').append(prp) }
})
...
async function save() {
    ...
    if (!d.includes(iscode)) {
        ...
        while (d.includes('%20%20')) { d = d.replaceAll('%20%20', '%20') }
    };
    ...
    if (dict == undefined) {
        ...
        if (!dict[url[2]]) { dict[url[2]] = { auth: 1 } };
        dict[url[2]][ss.edit] = d;
        if (dict[url[2]].auth < 2) { dict[url[2]][!ss.edit] = dict[url[2]].auth ? '' : d };
        await updateDoc(doc(db, url[0], url[1]), dict)
    };
    getData(ss.edit).then((html) => setData(html));
    if (ss.prp == 'true') { location.reload() }
}

function onEnterSignin() { if (event.keyCode == 13) { signin() } }

function signin() {
    signInWithEmailAndPassword(auth, $('#id').value, $('#pw').value).then(async(userCredential) => {
...
function _wresize() { if (/Android|iPhone|iPad|iPod/i.test(navigator.userAgent)) { if ($('section')) { $('section').classList.add('m-s') }; if ($('aside')) { $('aside').classList.add('m-a') }; if ($('nav')) { $('nav').classList.add('m-n') } } else { if ($('section')) { $('section').classList.remove('m-s') }; if ($('aside')) { $('aside').classList.remove('m-a') }; if ($('nav')) { $('nav').classList.remove('m-n') } } }
